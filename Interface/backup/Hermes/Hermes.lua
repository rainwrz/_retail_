...
function Hermes:LoadTalentDatabase(reset)
	...
	for classId = 1, classCount do
		...
		local talentCount = GetNumTalents(classId)
		...
		if #talents < talentCount or reset == true then
			...
			for talentIndex = 1, talentCount do
				local name, texture, tier, column, selected, available = GetTalentInfo(talentIndex, true, nil, nil, classId)
...
function core:InitializePlayer()
	Player.name, Player.realm = UnitName("player")
