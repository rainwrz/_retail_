...
local function LoopIntermission()
	if not southDestroyed then
		...
		sndWOP:Schedule(7, "Interface\\AddOns\\DBM-Core\\extrasounds\\ex_tt_czzb.mp3")
	end
	...
	if not northDestroyed then
		...
		sndWOP:Schedule(18, "Interface\\AddOns\\DBM-Core\\extrasounds\\ex_tt_jdzb.mp3")
	end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, _, _, spellId)
	if spellId == 137146 and self:AntiSpam(2, 2) then--Supercharge Conduits (comes earlier than other events so we use this one)
		...
		if not northDestroyed then
			...
			sndWOP:Schedule(16, "Interface\\AddOns\\DBM-Core\\extrasounds\\ex_tt_jdzb.mp3")
		end
