...
function HPetBattleAny.CreateLinkByInfo(petID,...)		---...=usecustom,level,health,power,speed,rarity
	...
	link=link..speciesID..":"..level..":"..rarity..":"..health..":"..power..":"..speed..":"..(petID or "0x0000000000000000")
...
function HPetBattleAny:GetPetCollectedInfo(speciesID,enemypet,islink,mini)
	...
	if pets then
		...
		if C_PetBattles.IsPlayerNPC(2) and (select(2,C_PetBattles.IsTrapAvailable())==6 or select(2,C_PetBattles.IsTrapAvailable())==7) then
			...
			if enemypet and not select(7,C_PetJournal.GetPetInfoBySpeciesID(speciesID)) then
...
--~ 				(pets) or
			...
			self.EnemyPetInfo[petIndex]={["speciesID"]=speciesID,["name"]=name,[1]=level,[2]=health,[3]=power,[4]=speed,[5]=rarity,[6]=isflying,["breedID"]=nil}
