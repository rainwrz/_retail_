...
local defaults = {
	profile = {
		...
		frame = {
			locked = false,
			texture = BossSwingTimerPath .. "glaze.tga",
			point = {
				x = 0,
				y = 0,
				p = "CENTER",
		...
		args = {
			...
			frame = {
				...
				args = {
					locked = {
						name = "Lock Frame",
						desc = "Locks/unlocks the BossSwingTimer frame (needs to be unchecked to enable dragging/moving)",
						order = 0,
						type = "toggle",
						width = "full",
						set = function(info,val)
					...
					width = {
						name = "Width",
						desc = "BossSwingTimer frame scale",
						order = 2.1,
						type = "range",
						min = 1,
						max = 500,
					...
					alpha = {
						name = "Alpha",
						desc = "BossSwingTimer background alpha",
						order = 2.5,
						type = "range",
						min = 0,
						max = 1,
...
function BossSwingTimer:OnUpdate()
	...
	lag = lag / 1000
	if lag > self.db.profile.frame.length then
		lag = self.db.profile.frame.length
	end
	self.bar.lag:SetWidth(lag / self.db.profile.frame.length * self.db.profile.frame.width)
	...
	for k, v in pairs(self.swings) do
		if v.next and v.next < time + self.db.profile.frame.length then
			if not v.tick then
			...
			v.tick:SetVertexColor(color.r, color.g, color.b, alpha)
			v.tick:SetPoint("LEFT", self.bar, "LEFT", (v.next - time) / self.db.profile.frame.length * self.db.profile.frame.width - 2, 0)
			if special[k] then
