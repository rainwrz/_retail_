...
DBMSkin:SetScript("OnEvent", function()
	...
	if IsAddOnLoaded("DBM-Core") then
		...
		self.shadow:SetBackdrop({
			edgeFile = "Interface\\AddOns\\addonskin\\media\\glowTex", 
			edgeSize = 5,
			...
			for bar in self:GetBarIterator() do
				if not bar.injected then
						...
						if not texture.styled then
							texture:SetTexture("Interface\\AddOns\\addonskin\\media\\statusbar")
							texture.styled=true
						end
				
						tbar:SetStatusBarTexture("Interface\\AddOns\\addonskin\\media\\statusbar")
		...
		local SkinBoss=function()
			...
			while (_G[format("DBM_BossHealth_Bar_%d", count)]) do
				...
				if not progress.styled then
					progress:SetStatusBarTexture("Interface\\AddOns\\addonskin\\media\\statusbar")
